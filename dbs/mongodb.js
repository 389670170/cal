...
function database(opts) {
  return clients[opts['client']].db(dbname(opts));
}
module.exports = {
  /**
  ...
  insertOne: function(connname, doc, opts = defaultCo) {
    return database(opts).collection(connname).insertOne(doc);
  },
  deleteOne:  function(connname, doc, opts = defaultCo) {
    return database(opts).collection(connname).deleteOne(doc);
  },
  findOne1: function(connname, query, options) {
    return mdb.collection(connname).findOne(query, options);
  },
  count: function(connname, query,opts = defaultCo) {
    return database(opts).collection(connname).count(query);
  },
  updateOne: (connname, filter, update, options,opts = defaultCo) => {
    return database(opts).collection(connname).updateOne(filter, update, options);
  },
  collection: (collname) => {
